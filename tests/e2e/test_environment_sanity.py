...
Runs first (alphabetically early, and trivially fast) so a broken browser
environment surfaces as ONE clear failure instead of dozens of cryptic
click-timeout failures plus an apparent suite hang.
"""
...
def test_animation_frames_fire(page: Page) -> None:
    ...
    Known cause (2026-06-10 incident): a stale ``DISPLAY`` env var — a
    devcontainer X-forwarding socket left over from a previous editor
    session — wedges headless chromium's frame production even though the
    page reports ``visibilityState === "visible"``.  ``tests/e2e/conftest.py``
    strips ``DISPLAY`` for headless runs; if this test fails, check for new
    env leakage into the browser environment (and that the conftest guard
    still runs before the browser launches).
    """
