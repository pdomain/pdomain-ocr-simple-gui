...
1. Encoded `project_id` path traversal can recursively delete the project store.
   - Evidence: `src/pdomain_ocr_simple_gui/storage.py:15`, `src/pdomain_ocr_simple_gui/routes/jobs.py:143`, `src/pdomain_ocr_simple_gui/routes/jobs.py:148`, `src/pdomain_ocr_simple_gui/storage.py:111`.
...
1. Arbitrary local image disclosure through caller-controlled `source_path`.
   - Evidence: `src/pdomain_ocr_simple_gui/routes/jobs.py:25`, `src/pdomain_ocr_simple_gui/routes/jobs.py:99`, `src/pdomain_ocr_simple_gui/pipeline.py:36`, `src/pdomain_ocr_simple_gui/routes/pages.py:81`, `src/pdomain_ocr_simple_gui/routes/pages.py:91`.
...
2. Unauthenticated OCR job and rerun endpoints allow CPU/GPU denial of service.
   - Evidence: `src/pdomain_ocr_simple_gui/routes/jobs.py:94`, `src/pdomain_ocr_simple_gui/routes/jobs.py:119`, `src/pdomain_ocr_simple_gui/pipeline.py:42`, `src/pdomain_ocr_simple_gui/pipeline.py:142`, `src/pdomain_ocr_simple_gui/routes/pages.py:112`, `src/pdomain_ocr_simple_gui/routes/pages.py:156`.
...
3. Mounted suite launch API allows unauthenticated local process spawning.
   - Evidence: `src/pdomain_ocr_simple_gui/app.py:94`, `/workspaces/ocr-container/pdomain-ocr-ops/pdomain_ocr_ops/suite/routes.py:47`, `/workspaces/ocr-container/pdomain-ocr-ops/pdomain_ocr_ops/suite/routes.py:55`, `/workspaces/ocr-container/pdomain-ocr-ops/pdomain_ocr_ops/suite/sibling_spawn.py:92`, `/workspaces/ocr-container/pdomain-ocr-ops/pdomain_ocr_ops/suite/sibling_spawn.py:93`.
...
4. Vite dev server path traversal advisory is present.
...
5. esbuild dev server CORS advisory is present via Vite.
...
6. Private `pdomain-book-tools` artifacts are locked without integrity hashes.
...
1. Project metadata and preferences are readable/writable without authentication.
   - Evidence: `src/pdomain_ocr_simple_gui/routes/jobs.py:123`, `src/pdomain_ocr_simple_gui/routes/jobs.py:128`, `src/pdomain_ocr_simple_gui/routes/prefs.py:14`, `src/pdomain_ocr_simple_gui/routes/prefs.py:28`, `/workspaces/ocr-container/pdomain-ocr-ops/pdomain_ocr_ops/suite/routes.py:58`, `/workspaces/ocr-container/pdomain-ocr-ops/pdomain_ocr_ops/suite/routes.py:63`.
...
2. Local UI loads third-party Google Fonts.
   - Evidence: `frontend/index.html:7`, `frontend/index.html:9`, `src/pdomain_ocr_simple_gui/frontend/index.html:7`, `src/pdomain_ocr_simple_gui/frontend/index.html:9`.
...
3. Output directory is exposed as a raw `file://` href.
...
4. Suite launcher opens new tabs without `noopener`.
    - Evidence: `frontend/src/App.tsx:91` enables the launcher; `frontend/node_modules/@pdomain/pdomain-ui/dist/RightPanel-Z4PwHl58.js:19` calls `window.open(url, "_blank")`.
...
5. `pdomain-ocr-ops` is an unversioned editable sibling dependency.
    ...
    - Recommendation: depend on a versioned `pdomain-ocr-ops` artifact or pinned git commit; keep editable use as a local override.

6. CI uses mutable action/tool references.
...
7. Bandit B110: suite unregister failure is silently swallowed.
    - Evidence: `src/pdomain_ocr_simple_gui/__main__.py:52`.
...
8. Bandit B110: suite self-registration failure is silently swallowed.
    - Evidence: `src/pdomain_ocr_simple_gui/app.py:61`.
...
9. Bandit B110: suite route mounting failure is silently swallowed.
    - Evidence: `src/pdomain_ocr_simple_gui/app.py:99`.
...
10. Bandit B110: failed-status persistence failure is silently swallowed.
    - Evidence: `src/pdomain_ocr_simple_gui/routes/jobs.py:90`.
...
11. Bandit B110: recent-project preference update failure is silently swallowed.
    - Evidence: `src/pdomain_ocr_simple_gui/routes/jobs.py:200`.
...
12. Bandit B110: unreadable project directories are silently skipped.
    - Evidence: `src/pdomain_ocr_simple_gui/storage.py:106`.
